...
#	color	: Progress character color						... 0=Black 1=Red 2=Green 3=Yellow 4=Blue 5=Purple 6=Cyan 7=White
# ------------------------------------------------
import sys
def updateBar(curVal, maxVal, prefix="Progress", suffix="", width=40, mark="#", color="2"):
	prog = int(width * curVal // float(maxVal))
	sys.stdout.write("\r%s : [ \x1B[3%sm%s\x1B[0m ] %d%% %s" % (str(prefix), color, mark * prog + "." * (width - prog), 100 * (curVal / float(maxVal)), str(suffix)))
...
for curVal in range(minVal, maxVal + 1):
	...
	# Update Progress-Bar
	updateBar(curVal, maxVal, color=4)		# 0=Black 1=Red 2=Green 3=Yellow 4=Blue 5=Purple 6=Cyan 7=White
